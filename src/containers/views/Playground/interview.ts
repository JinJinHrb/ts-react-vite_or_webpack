...
export const mySimpleDebounce = function (fn: () => any, delay: number) {
...
export const myDebounce = function (fn: () => any, delay: number, immediate: boolean) {
	let timer: NodeJS.Timeout, result: any
	const debounce = function (...arg: any) {
		// eslint-disable-next-line @typescript-eslint/no-this-alias
		const ctx = this
		if (timer) {
			clearTimeout(timer)
...
export const myCurry = fn => {
	const handler = (...arg) => {
		if (arg.length === fn.length) {
			return fn(...arg)
		}
		return (...param) => handler(...arg, ...param)
	}
	return handler
}

// 快速排序是一种非常高效的排序算法，其基本思想是选择一个"主元"（pivot），然后把数组分为两部分，使得左边的元素都比主元小，右边的元素都比主元大。然后对左右两部分递归地进行快速排序。
// 以下是一个用JavaScript实现的快速排序的例子：
...
// console.log(quickSort(arr));  // 输出: [ 17, 24, 31, 45, 50, 63, 85, 96 ]
